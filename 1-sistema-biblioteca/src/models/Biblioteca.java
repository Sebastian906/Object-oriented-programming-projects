...
import service.GeneradorIdService;
...
public class Biblioteca {
     ...
     * @throws LibroNoDisponibleException    si no hay ejemplares disponibles
     * @throws UsuarioNoEncontradoException  si no existe el usuario
     * @throws UsuarioInactivoException      si el usuario no está activo
     * @throws LimitePrestamosException      si el usuario alcanzó el límite de préstamos
     */
    public Prestamo prestarLibro(String isbn, String idUsuario, int diasPrestamo) 
        ...
        if (!usuario.estaActivo()) {
            throw new UsuarioInactivoException(usuario.getNombre());
        }

        // 5. Verificar límite de préstamos activos (máximo 3)
        int prestamosActivos = contarPrestamosActivos(idUsuario);
        if (prestamosActivos >= 3) {
            throw new LimitePrestamosException(usuario.getNombre());
        }

        // 6. Crear el préstamo (usando servicio de generación de IDs)
        String idPrestamo = GeneradorIdService.generarIdPrestamo();
     ...
     * @throws PrestamoNoEncontradoException si no existe el préstamo
     * @throws PrestamoYaDevueltoException   si el préstamo ya fue devuelto
     */
    public String devolverLibro(String idPrestamo) 
            throws PrestamoNoEncontradoException, PrestamoYaDevueltoException {
     ...
     * Utiliza el servicio de generación de IDs.
     * @return ID de usuario único
     */
    public String generarIdUsuario() {
        return GeneradorIdService.generarIdUsuario();
    }
